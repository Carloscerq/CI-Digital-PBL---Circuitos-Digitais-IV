#!/usr/bin/env bash
# Runs the spi UVM testbench with Verilator (open source, no licence needed).
...
#   ./run_uvm_verilator.sh +UVM_TESTNAME=spi_directed_test
#   ./run_uvm_verilator.sh --trace                    # FST waves for gtkwave
#   ./run_uvm_verilator.sh --build-only               # verilate + compile only
#
# All work happens in RTL/sim_verilator/ (log: RTL/sim_verilator/spi.log).
...
exec "$dir/scripts/uvm_verilator.sh" spi "$@"
