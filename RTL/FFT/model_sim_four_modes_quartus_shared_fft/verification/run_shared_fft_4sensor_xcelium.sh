...
output_dir=${OUTPUT_DIR:-$model_root/results_shared_fft_4sensor_xcelium}
...
((${#selected_cases[@]} > 0)) || \
    fail "nenhum caso corresponde a CASE_PATTERN=$case_pattern"
...
manifest=$output_dir/shared_fft_4sensor_manifest.csv
printf '%s\n' 'case,hop_size,max_frames,status,bins_csv,report' > "$manifest"
...
for relative_case in "${selected_cases[@]}"; do
    ...
    if [[ -z "$sensor1_file" || -z "$sensor2_file" ||
        ...
        printf '[XCELIUM][SHARED FFT] caso=%s hop=%s\n' \
            "$relative_case" "$hop_size"
        xrun -64bit -sv +define+RTL_SIM -timescale 1ns/1ps -access +rwc \
            -xmlibdirname "$build_dir" \
            -top tb_shared_fft_4sensor_dataset \
            -defparam "tb_shared_fft_4sensor_dataset.HOP_SIZE=$hop_size" \
            "${sources[@]}" \
    ...
    printf '%s,%s,%s,%s,%s,%s\n' \
        "$relative_case" "$hop_size" "$max_frames" "$status" \
        "$bins_csv" "$report_file" >> "$manifest"
done

printf 'SHARED FFT FINALIZADA: casos=%d falhas=%d manifesto=%s\n' \
