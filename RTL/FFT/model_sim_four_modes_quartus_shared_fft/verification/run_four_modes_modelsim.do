# =============================================================================
# ModelSim/Questa 2020.1 compativel.
# =============================================================================

transcript on
onerror {resume}

if {![info exists PROJECT_ROOT]} {set PROJECT_ROOT [file normalize [pwd]]}
if {![info exists DATASET_Q915]} {set DATASET_Q915 [file join $PROJECT_ROOT dataset_q915]}
if {![info exists DATASET_Q1116]} {set DATASET_Q1116 [file join $PROJECT_ROOT dataset_q1116]}
if {![info exists OUTPUT_DIR]} {set OUTPUT_DIR [file join $PROJECT_ROOT results_four_modes]}
if {![info exists CASE_PATTERN]} {set CASE_PATTERN "*"}
if {![info exists MAX_CASES]} {set MAX_CASES 1}
if {![info exists MAX_FRAMES]} {set MAX_FRAMES 10}
if {![info exists DESIRED_SENSOR]} {set DESIRED_SENSOR 1}
if {![info exists REFERENCE_SENSOR]} {set REFERENCE_SENSOR 2}
if {![info exists ADAPT_SAMPLES]} {set ADAPT_SAMPLES 0}
if {![info exists MU_SHIFT]} {set MU_SHIFT 16}
if {![info exists HOP_SIZE]} {set HOP_SIZE 8}
if {![info exists INPUT_SAMPLE_RATE_HZ]} {set INPUT_SAMPLE_RATE_HZ 25600}
if {![info exists SAVE_ALL_BINS]} {set SAVE_ALL_BINS 0}
if {![info exists PROGRESS_FRAMES]} {set PROGRESS_FRAMES 10}
...
set manifest_path [file join $OUTPUT_DIR four_modes_manifest_sensor${DESIRED_SENSOR}.csv]
...
foreach relative_case $selected_cases {
    ...
    foreach mode_def $mode_table {
        ...
        set mode_output [file join $OUTPUT_DIR $relative_case sensor${DESIRED_SENSOR} $mode]
