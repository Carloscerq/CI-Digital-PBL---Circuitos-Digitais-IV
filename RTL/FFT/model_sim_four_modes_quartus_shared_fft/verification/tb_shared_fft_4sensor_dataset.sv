...
// Teste integrado Q9.15 sem LMS para quatro sensores sincronizados.
// Cada linha temporal dos quatro arquivos e entregue atomicamente ao DUT.
module tb_shared_fft_4sensor_dataset #(
        ...
        if (reset) begin
            ...
            if (fft_valid && fft_ready) begin
                ...
                if (fft_bin == 6'd63) begin
                    ...
                    $display(
                        "[SHARED_FFT] sensor=%0d frame=%0d concluido",
                        output_sensor + 1,
                        frame_count[output_sensor] + 1);
                end
                else
    ...
    initial begin
        ...
        $fwrite(report_fd, "frames_sensor1=%0d\n", frame_count[0]);
        $fwrite(report_fd, "frames_sensor2=%0d\n", frame_count[1]);
        $fwrite(report_fd, "frames_sensor3=%0d\n", frame_count[2]);
        $fwrite(report_fd, "frames_sensor4=%0d\n", frame_count[3]);
        $fwrite(report_fd, "overflow_sensor1=%0d\n", overflow_count[0]);
        $fwrite(report_fd, "overflow_sensor2=%0d\n", overflow_count[1]);
        $fwrite(report_fd, "overflow_sensor3=%0d\n", overflow_count[2]);
        $fwrite(report_fd, "overflow_sensor4=%0d\n", overflow_count[3]);
        $fwrite(report_fd, "status=PASS\n");

        $display(
            "[SHARED_FFT] PASS: entradas=%0d frames_por_sensor=%0d",
            input_samples, expected_frames);
