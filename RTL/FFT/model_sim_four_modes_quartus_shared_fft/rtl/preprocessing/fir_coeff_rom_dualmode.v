`timescale 1ns/1ps

// ROM sincrona para coeficientes FIR signed Q1.17.
// Os coeficientes permanecem Q1.17 tanto no modo Q9.15 quanto no Q11.16.
module fir_coeff_rom_dualmode #(
    parameter integer COEFF_WIDTH = 18,
    parameter integer ADDR_WIDTH  = 7,
    parameter integer NUM_TAPS    = 47,
	parameter INIT_FILE =
	    "../coefficients/fir/stage1_decim4_q117.bin"
)(
    input  wire                          clk,
    input  wire                          reset,
    input  wire                          read_enable,
    input  wire [ADDR_WIDTH-1:0]         coeff_addr,
    output reg signed [COEFF_WIDTH-1:0]  coeff_out,
    output reg                           coeff_valid,
    output reg                           addr_error
);

    // O Quartus implementa ROMs com profundidade fisica em potencia de dois.
    // Declarar essa profundidade explicitamente, e preencher o arquivo ate ela,
    // evita a divergencia 47/64, 67/128 e 83/128 nos MIFs internos.
    localparam integer ROM_DEPTH =
        (NUM_TAPS <= 1) ? 1 : (1 << $clog2(NUM_TAPS));

    (* romstyle = "logic" *)
    reg signed [COEFF_WIDTH-1:0] coeff_mem [0:ROM_DEPTH-1];
    integer init_index;

    // Verificacoes de arquivo usadas apenas em simulacao. As diretivas
    // translate_off/on sao reconhecidas pelo Quartus independentemente da
    // definicao externa da macro SYNTHESIS.
    // synthesis translate_off
    integer coefficient_file;
    // synthesis translate_on

    initial begin
        for (init_index = 0; init_index < ROM_DEPTH;
             init_index = init_index + 1)
            coeff_mem[init_index] = {COEFF_WIDTH{1'b0}};

        // synthesis translate_off
        if (INIT_FILE == "")
            $fatal(1, "[fir_coeff_rom_dualmode] INIT_FILE vazio.");

        coefficient_file = $fopen(INIT_FILE, "r");
        if (coefficient_file == 0)
            $fatal(1,
                "[fir_coeff_rom_dualmode] Arquivo nao encontrado: %0s",
                INIT_FILE);
        $fclose(coefficient_file);
        // synthesis translate_on

        if (INIT_FILE != "")
            $readmemb(INIT_FILE, coeff_mem);
    end

    always @(posedge clk) begin
        if (reset) begin
            coeff_out   <= {COEFF_WIDTH{1'b0}};
            coeff_valid <= 1'b0;
            addr_error  <= 1'b0;
        end
        else begin
            coeff_valid <= 1'b0;
            addr_error  <= 1'b0;

            if (read_enable) begin
                if (coeff_addr < NUM_TAPS) begin
                    coeff_out   <= coeff_mem[coeff_addr];
                    coeff_valid <= 1'b1;
                end
                else begin
                    coeff_out   <= {COEFF_WIDTH{1'b0}};
                    addr_error  <= 1'b1;
                end
            end
        end
    end

    // synthesis translate_off
    initial begin
        if (COEFF_WIDTH != 18)
            $warning(
                "[fir_coeff_rom_dualmode] COEFF_WIDTH=%0d; arquivos fornecidos usam Q1.17/18 bits.",
                COEFF_WIDTH);
        if (NUM_TAPS <= 0)
            $fatal(1, "[fir_coeff_rom_dualmode] NUM_TAPS invalido.");
        if ((1 << ADDR_WIDTH) < ROM_DEPTH)
            $fatal(1,
                "[fir_coeff_rom_dualmode] ADDR_WIDTH nao representa ROM_DEPTH.");
    end
    // synthesis translate_on

endmodule
