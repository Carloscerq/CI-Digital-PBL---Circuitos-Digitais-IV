#!/usr/bin/env bash
# Runs the LMS+FFT preprocessing pipeline UVM testbench with Questa/ModelSim (vsim).
# Same DUT config, top module (preprocess_lms_fft_uvm_top) and default test as run_uvm.sh,
# which drives Xcelium instead.
#
#   ./run_uvm_vsim.sh                                # default test
#   ./run_uvm_vsim.sh +UVM_VERBOSITY=UVM_HIGH        # per-item logging
#   ./run_uvm_vsim.sh +UVM_TESTNAME=preprocess_lms_fft_directed_test
#   ./run_uvm_vsim.sh --gui                          # interactive, waves kept
#
# All work happens in RTL/sim_vsim/ (log: RTL/sim_vsim/fft.log).
set -uo pipefail
...
exec "$dir/scripts/uvm_vsim.sh" fft "$@"
