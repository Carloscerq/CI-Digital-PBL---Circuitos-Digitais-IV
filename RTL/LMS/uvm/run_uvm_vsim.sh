#!/usr/bin/env bash
# Runs the filtro_lms (MU=24'sd1638) UVM testbench with Questa/ModelSim (vsim).
# Same DUT config, top module (filtro_lms_uvm_top) and default test as run_uvm.sh,
# which drives Xcelium instead.
#
#   ./run_uvm_vsim.sh                                # default test
#   ./run_uvm_vsim.sh +UVM_VERBOSITY=UVM_HIGH        # per-item logging
#   ./run_uvm_vsim.sh +UVM_TESTNAME=filtro_lms_regression_test
#   ./run_uvm_vsim.sh --gui                          # interactive, waves kept
#
# All work happens in RTL/sim_vsim/ (log: RTL/sim_vsim/filtro_lms.log).
set -uo pipefail
...
exec "$dir/scripts/uvm_vsim.sh" filtro_lms "$@"
