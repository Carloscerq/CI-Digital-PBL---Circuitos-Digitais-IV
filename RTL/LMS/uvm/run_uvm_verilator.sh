#!/usr/bin/env bash
# Runs the filtro_lms UVM testbench with Verilator (open source, no licence needed).
...
#   ./run_uvm_verilator.sh +UVM_TESTNAME=filtro_lms_regression_test
#   ./run_uvm_verilator.sh --trace                    # FST waves for gtkwave
#   ./run_uvm_verilator.sh --build-only               # verilate + compile only
#
# All work happens in RTL/sim_verilator/ (log: RTL/sim_verilator/filtro_lms.log).
...
exec "$dir/scripts/uvm_verilator.sh" filtro_lms "$@"
