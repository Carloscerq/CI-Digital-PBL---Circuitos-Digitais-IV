#!/usr/bin/env bash
# Runs the cnn_mac_q8_16 UVM testbench with Verilator (open source, no licence needed).
...
#   ./run_uvm_verilator.sh +UVM_TESTNAME=mac_q8_16_default_test
#   ./run_uvm_verilator.sh --trace                    # FST waves for gtkwave
#   ./run_uvm_verilator.sh --build-only               # verilate + compile only
#
# All work happens in RTL/sim_verilator/ (log: RTL/sim_verilator/cnn_mac_q8_16.log).
...
exec "$dir/scripts/uvm_verilator.sh" cnn_mac_q8_16 "$@"
