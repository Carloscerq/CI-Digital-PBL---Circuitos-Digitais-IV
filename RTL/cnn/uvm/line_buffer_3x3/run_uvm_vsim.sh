#!/usr/bin/env bash
# Runs the 3x3 line buffer UVM testbench with Questa/ModelSim (vsim).
# Same DUT config, top module (line_buffer_3x3_uvm_top) and default test as run_uvm.sh,
# which drives Xcelium instead.
#
#   ./run_uvm_vsim.sh                                # default test
#   ./run_uvm_vsim.sh +UVM_VERBOSITY=UVM_HIGH        # per-item logging
#   ./run_uvm_vsim.sh +UVM_TESTNAME=line_buffer_3x3_directed_random_test
#   ./run_uvm_vsim.sh --gui                          # interactive, waves kept
#
# All work happens in RTL/sim_vsim/ (log: RTL/sim_vsim/cnn_line_buffer.log).
set -uo pipefail
...
exec "$dir/scripts/uvm_vsim.sh" cnn_line_buffer "$@"
