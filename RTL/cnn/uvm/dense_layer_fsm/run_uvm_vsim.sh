#!/usr/bin/env bash
# Runs the dense layer FSM UVM testbench with Questa/ModelSim (vsim).
# Same DUT config, top module (dense_layer_fsm_uvm_top) and default test as run_uvm.sh,
# which drives Xcelium instead.
#
#   ./run_uvm_vsim.sh                                # default test
#   ./run_uvm_vsim.sh +UVM_VERBOSITY=UVM_HIGH        # per-item logging
#   ./run_uvm_vsim.sh +UVM_TESTNAME=dense_layer_fsm_directed_random_test
#   ./run_uvm_vsim.sh --gui                          # interactive, waves kept
#
# All work happens in RTL/sim_vsim/ (log: RTL/sim_vsim/cnn_dense.log).
set -uo pipefail
...
exec "$dir/scripts/uvm_vsim.sh" cnn_dense "$@"
