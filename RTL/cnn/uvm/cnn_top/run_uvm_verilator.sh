#!/usr/bin/env bash
# Runs the cnn_top UVM testbench with Verilator (open source, no licence needed).
...
#
# All work happens in RTL/sim_verilator/ (log: RTL/sim_verilator/cnn_top.log).
...
exec "$dir/scripts/uvm_verilator.sh" cnn_top "$@"
