// ---------------------------------------------------------------------
//  cnn_top_uvm_top  --  DUT + interface + UVM entry point for the
...
//  10ns period, same as tb_cnn_top.sv), the DUT/interface
...
//  reset_phase, driven by cnn_top_driver (which owns the vif) --
//  see cnn_top_driver.sv. That keeps stimulus timing entirely
...
//  tb_cnn_top.sv applies -- reset high across the first
//  RESET_CYCLES posedges, deasserted mid-cycle -- just expressed as a
//  phase instead of an `initial ... #22`.
//
//  There's exactly one valid geometry here (see cnn_top_pkg.sv),
...
//
//  dense_data_probe wiring: cnn_top_scoreboard.sv needs to verify
...
//  order inside cnn_top.sv, not just that they hold *some* stable
...
//  directly in cnn_top's own module body (not inside a generate
...
//  into the DUT -- only reads dense_data -- so cnn_top.sv itself is
//  untouched.
// ---------------------------------------------------------------------
`timescale 1ns/1ps

module cnn_top_uvm_top;
    ...
    initial begin
        uvm_config_db #(virtual cnn_top_if)::set(null, "uvm_test_top.env.agent.*", "vif", vif);
