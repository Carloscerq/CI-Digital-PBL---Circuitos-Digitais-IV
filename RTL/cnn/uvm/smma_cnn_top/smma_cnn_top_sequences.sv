// ---------------------------------------------------------------------
//  smma_cnn_top_directed_seq  --  loads the REAL spectrogram frame
//  tb_smma_cnn_top.sv uses, Scripts/cnn/cnn_tb_input.mem (repo-root
//  relative; resolves correctly once run_uvm.sh has cd'd into RTL/, so
//  the relative path here, "../Scripts/cnn/cnn_tb_input.mem", is
//  identical to the literal path tb_smma_cnn_top.sv itself uses), and
...
//  tb_smma_cnn_top.sv's own `tb_image_data[(i*4)+ch]` indexing uses,
//  just unflattened into pixels[r][c][ch] here (i = r*IMG_WIDTH + c).
//
//  smma_cnn_top_random_seq  --  a small handful of frames (default 1)
...
//  whole default test.
// ---------------------------------------------------------------------
class smma_cnn_top_directed_seq extends uvm_sequence #(smma_cnn_top_seq_item);

    `uvm_object_utils(smma_cnn_top_directed_seq)

    function new(string name = "smma_cnn_top_directed_seq");
        super.new(name);
    endfunction
...
class smma_cnn_top_random_seq extends uvm_sequence #(smma_cnn_top_seq_item);

    `uvm_object_utils(smma_cnn_top_random_seq)

    int unsigned num_frames = 1;

    function new(string name = "smma_cnn_top_random_seq");
        super.new(name);
    endfunction
    ...
    task body();
        ...
        repeat (num_frames) begin
            item = smma_cnn_top_seq_item::type_id::create("item");
            if (!item.randomize())
                `uvm_fatal("RAND", "failed to randomize smma_cnn_top_seq_item frame")
            start_item(item);
            finish_item(item);
        end
    endtask
