#!/bin/sh
# Runs the smma_cnn_top UVM testbench (CNN top-level integration: the
...
# ($readmemh("../Scripts/cnn/cnn_tb_input.mem") in smma_cnn_top_sequences.sv)
...
#   ./run_uvm.sh +UVM_TESTNAME=smma_cnn_top_directed_random_test

cd "$(dirname "$0")/../../.."

xrun -64bit -sv -uvmhome CDNS-1.2 -timescale 1ns/1ps -access +rwc \
     -incdir cnn/uvm/smma_cnn_top \
     -top smma_cnn_top_uvm_top \
     -f cnn/uvm/smma_cnn_top/smma_cnn_top_uvm.files \
     +UVM_TESTNAME=smma_cnn_top_directed_random_test \
     "$@"
