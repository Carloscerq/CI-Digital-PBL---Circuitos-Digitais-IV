// ---------------------------------------------------------------------
//  smma_cnn_top_if  --  connects the UVM agent to smma_cnn_top, the
...
//  default geometry tb_smma_cnn_top.sv exercises, and the only
...
//  (rather than imported from smma_cnn_top_pkg) because this interface,
//  like every interface in this repo, is compiled as its own
//  standalone unit ahead of the package -- keep these in sync with
//  smma_cnn_top_pkg.sv by hand if the geometry ever changes.
// ---------------------------------------------------------------------
interface smma_cnn_top_if (
    input logic clk
);
    ...
    // Internal-signal probe -- NOT a DUT port. smma_cnn_top_uvm_top.sv
    ...
    // smma_cnn_top.sv), so the scoreboard can verify the breakout wiring
    ...
    // stable value. See smma_cnn_top_scoreboard.sv for the check and
    // smma_cnn_top_uvm_top.sv for why this hierarchical-reference
