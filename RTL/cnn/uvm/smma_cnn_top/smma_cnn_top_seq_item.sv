// ---------------------------------------------------------------------
//  smma_cnn_top_seq_item  --  one whole frame of stimulus: every pixel
...
//  tb_smma_cnn_top.sv's raster-order flattening (tb_image_data[(i*4)+ch])
//  and line_buffer_3x3_seq_item's own [r][c][ch] convention.
//
//  This testbench is a protocol/integration-level testbench, not a
//  bit-exact one (see smma_cnn_top_scoreboard.sv for why -- the 4
...
class smma_cnn_top_seq_item extends uvm_sequence_item;
    ...
    logic signed [DATA_WIDTH-1:0] probe_dense [OUT_CLASSES]; // dense_data[0..3], see smma_cnn_top_if.sv
    bit                           last;
    int unsigned                  input_beats_accepted; // independently counted on s_axis, see monitor

    `uvm_object_utils(smma_cnn_top_seq_item)

    function new(string name = "smma_cnn_top_seq_item");
        super.new(name);
    endfunction

    function string convert2string();
        return $sformatf("smma_cnn_top_seq_item: frame=%0dx%0dx%0d | logits(n,u,mi,b)=(%0d,%0d,%0d,%0d) last=%0b beats=%0d",
    ...
    function void do_copy(uvm_object rhs);
        smma_cnn_top_seq_item rhs_;
        if (!$cast(rhs_, rhs))
            `uvm_fatal("DO_COPY", "cast to smma_cnn_top_seq_item failed")
        super.do_copy(rhs);
        pixels                = rhs_.pixels;
