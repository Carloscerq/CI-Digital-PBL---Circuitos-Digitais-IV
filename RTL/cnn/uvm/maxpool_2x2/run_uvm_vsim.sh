#!/usr/bin/env bash
# Runs the 2x2 maxpool UVM testbench with Questa/ModelSim (vsim).
# Same DUT config, top module (maxpool_2x2_uvm_top) and default test as run_uvm.sh,
# which drives Xcelium instead.
#
#   ./run_uvm_vsim.sh                                # default test
#   ./run_uvm_vsim.sh +UVM_VERBOSITY=UVM_HIGH        # per-item logging
#   ./run_uvm_vsim.sh +UVM_TESTNAME=maxpool_2x2_directed_random_test
#   ./run_uvm_vsim.sh --gui                          # interactive, waves kept
#
# All work happens in RTL/sim_vsim/ (log: RTL/sim_vsim/cnn_maxpool.log).
set -uo pipefail
...
exec "$dir/scripts/uvm_vsim.sh" cnn_maxpool "$@"
