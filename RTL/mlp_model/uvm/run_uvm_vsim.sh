#!/usr/bin/env bash
# Runs the mlp UVM testbench with Questa/ModelSim (vsim).
# Same DUT config, top module (mlp_uvm_top) and default test as run_uvm.sh,
# which drives Xcelium instead.
#
#   ./run_uvm_vsim.sh                                # default test
#   ./run_uvm_vsim.sh +UVM_VERBOSITY=UVM_HIGH        # per-item logging
#   ./run_uvm_vsim.sh +UVM_TESTNAME=mlp_directed_random_test
#   ./run_uvm_vsim.sh --gui                          # interactive, waves kept
#
# All work happens in RTL/sim_vsim/ (log: RTL/sim_vsim/mlp.log).
set -uo pipefail
...
exec "$dir/scripts/uvm_vsim.sh" mlp "$@"
