#!/usr/bin/env bash
# Runs the spectrogram UVM testbench with Verilator (open source, no licence needed).
...
#   ./run_uvm_verilator.sh +UVM_TESTNAME=spectrogram_generator_directed_random_test
#   ./run_uvm_verilator.sh --trace                    # FST waves for gtkwave
#   ./run_uvm_verilator.sh --build-only               # verilate + compile only
#
# All work happens in RTL/sim_verilator/ (log: RTL/sim_verilator/spectrogram.log).
...
exec "$dir/scripts/uvm_verilator.sh" spectrogram "$@"
