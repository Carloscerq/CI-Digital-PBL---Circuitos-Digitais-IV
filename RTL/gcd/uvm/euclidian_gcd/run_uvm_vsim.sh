#!/usr/bin/env bash
# Runs the pairwise Euclidean GCD (SIZE=32) UVM testbench with Questa/ModelSim (vsim).
# Same DUT config, top module (euclidian_gcd_uvm_top) and default test as run_uvm.sh,
# which drives Xcelium instead.
#
#   ./run_uvm_vsim.sh                                # default test
#   ./run_uvm_vsim.sh +UVM_VERBOSITY=UVM_HIGH        # per-item logging
#   ./run_uvm_vsim.sh +UVM_TESTNAME=euclidian_gcd_random_test
#   ./run_uvm_vsim.sh --gui                          # interactive, waves kept
#
# All work happens in RTL/sim_vsim/ (log: RTL/sim_vsim/euclidian_gcd.log).
set -uo pipefail
...
exec "$dir/scripts/uvm_vsim.sh" euclidian_gcd "$@"
